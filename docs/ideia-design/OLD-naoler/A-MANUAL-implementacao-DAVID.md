...
- **Os tijolos** = *blocos* (arvore, grafo, tabela, cartoes, filtros...). Pecas prontas,
  reutilizaveis.
- **A planta** = o layout que montas no **Claude Design** (a ferramenta visual da
  Anthropic). Arrastas as zonas — "aqui uma tabela, ali uma arvore, a esquerda filtros".
  Mas atencao: montas com **silhuetas** dos blocos (uns marcadores leves), nao com os
  blocos a serio. O Claude Design e um *gerador* — se lhe meteres o bloco verdadeiro, ele
  reinterpreta-o e estraga-o. As silhuetas servem so para desenhares a disposicao.
- **O empreiteiro** = o Claude Code, que le o teu layout do Claude Design, **troca cada
  silhueta pelo bloco real** que guarda na biblioteca, liga aos dados e ergue o edificio
...
- **A planta e a linguagem.** Montas o layout no Claude Design (com silhuetas), e e isso
  que comunica a mudanca ao Code — nao prosa solta. Os blocos a serio nunca entram no
  Claude Design; vivem no Code e so se juntam ao layout no fim.
