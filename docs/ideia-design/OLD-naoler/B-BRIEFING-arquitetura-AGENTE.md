...
```
   LAYOUT BASE                TU (Claude Code)            RESULTADO
   (Claude Design)            -----------------           ---------------
   o David monta as zonas  -> les o layout exportado  ->  HTML single-file
   com SILHUETAS dos          + substituis cada           final, offline,
   blocos -> exporta          silhueta pelo BLOCO         a funcionar
   layout/handoff             REAL da biblioteca
                              + injectas os dados
```

Principio nuclear: **a planta declara, tu constróis.** Os blocos reais vivem na
biblioteca do projecto (em ti), NUNCA dentro do Claude Design (que so ve silhuetas). O
Claude Design produz o layout base; tu fazes o edificio.
...
- **Manifesto / planta (`workspace.json`)** — declara o layout e que blocos. E a
  ...
  prosa.
- **Editor de planta = Claude Design** (decisao SArq-3). O David monta o **layout base**
  no Claude Design (ferramenta visual da Anthropic: descreve/arrasta zonas, exporta HTML /
  handoff para Claude Code). NAO se constroi editor proprio (Gridstack arquivado). Regra
  critica: **os blocos reais NUNCA entram no Claude Design** — ele e um *gerador* que
  reinterpreta o que recebe e corromperia o codigo do bloco. So entram **silhuetas**.
- **Silhueta de bloco** — representacao leve e descartavel de um bloco (do retangulo
  etiquetado ao mini-preview do aspecto), que TU geras para o David montar o layout no
  Claude Design. **Recomendas a fidelidade conforme o bloco** (um retangulo "BLOCO: grafo"
  pode bastar; um bloco cujo tamanho/proporcao afecta o layout pode pedir mini-preview).
  No fim, le o layout exportado e **substitui cada silhueta pelo bloco real** da
  biblioteca. As silhuetas sao andaime: nascem para o layout, morrem depois.
...
- **Silhuetas para o Claude Design.** O editor de planta esta decidido: e o **Claude
  Design** (ver sec. 2). NAO e tensao aberta. O que recomendas no grill e a **fidelidade
  das silhuetas** por tipo de bloco (retangulo etiquetado vs. mini-preview), e como geras
  o fluxo: silhuetas -> David monta layout no Claude Design -> exporta -> tu substituis
  pelos blocos reais. Valida na pratica o passo de import/export (o Claude Design e
  research preview; confirma que o layout exportado e legivel e estavel antes de assentar
  o fluxo nele).
