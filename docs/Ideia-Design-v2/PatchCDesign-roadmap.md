...
status: patch
summary: >
  Patch para roadmap.md: actualiza M2 (tensao Editor com 4 candidatos) e M9 (passo do
...
```
Editor (4 candidatos a avaliar: (a) manifesto-em-texto — recomendado como arranque, prova o
pipeline sem risco; (b) Claude Design + silhuetas / D-sil — ver ADDON-SArq3-Claude-Design.md;
(c) D-proc / Procreate — ver fluxo-D-proc.svg, paralelo; (d) Gridstack/próprio). Recomendar
com trade-offs; decisão do David.
```
...
- Se for **manifesto-em-texto** ou **D-proc** ou **Gridstack**: conforme trade-offs fechados no grill.
```
